...
from backend.broker_api.core.redis_events import get_redis, publish_event, pubsub_iter
...
from backend.broker_api.orders.order_runtime import PositionPnL, order_event_runtime, realtime_positions_service
from backend.broker_api.orders.worker_execution_links import worker_execution_links_store
from backend.broker_api.session.kite_session import KiteSession, get_kite, get_kite_session_id, get_session_account_id
...
class OrdersService:
    ...
    async def place_order(
        ...
        try:
            ...
            if session_id:
                from backend.broker_api.orders.live_order_intents import create_live_order_intent, validate_live_order_attribution
                from backend.execution_accounting.kite_costs import build_live_order_cost_contract
            ...
            if attribution and attribution.client_order_ref and e.status_code in {400, 401, 403, 404, 409, 422}:
                try:
                    from backend.broker_api.orders.live_order_intents import mark_live_order_intent_failed
