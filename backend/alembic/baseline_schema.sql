-- ===========================================================================
-- FROZEN Alembic baseline snapshot — DO NOT ADD FEATURES HERE.
--
-- This file is what migration 20260330_000001 executes to bootstrap an empty
-- database. It is a frozen snapshot of backend/schema.sql as of that revision,
-- with every block that a LATER migration owns removed:
--
--   * universes / universe_revisions          (migration 20260909_000014)
--   * screener runs + attachment state, and the signal_events workflow
--     columns                                 (migration 20260910_000015)
--   * Phase 4 advanced-condition, breadth, session-cap and external-signal
--     tables                                  (migration 20260911_000016)
--
-- Those blocks used to live in backend/schema.sql, which the baseline executed
-- verbatim. Because schema.sql keeps evolving, a from-zero `alembic upgrade
-- head` would run statements that reference tables a later migration in the
-- same chain creates, and abort at the first of them (recorded as defect D-2).
--
-- New schema changes belong in a NEW migration plus backend/schema.sql (the
-- evolving reference DDL used for direct from-scratch installs); never here.
-- ===========================================================================
