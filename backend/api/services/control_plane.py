...
from backend.api.services.control_plane_protection import ControlPlaneProtectionService, InvestingProtectionRepository
...
async def _exit_worker_control_strategy(request: Any, run: Dict[str, Any], *, reason: Optional[str], dry_run: bool, idempotency_key: Optional[str] = None) -> Dict[str, Any]:
    from backend.api.schemas.worker import WorkerExitRequest
    from backend.api.repositories.algo_worker_repo import WorkerToken
    from backend.api.routers.worker_execution import _exit_live_worker_run
