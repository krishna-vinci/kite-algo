...
```
<span><span>uvx mcpo --port 8000 -- your_mcp_server_command</span><br></span>
```

Or using `pip`:

```
<span><span>pip install mcpo</span><br></span><span><span>mcpo --port 8000 -- your_mcp_server_command</span><br></span>
```
...
> 
> ```
> <span><span>"mcpServers"</span><span>:</span><span> </span><span>{</span><span>   </span><br></span><span><span>  </span><span>"time"</span><span>:</span><span> </span><span>{</span><span>     </span><br></span><span><span>    </span><span>"command"</span><span>:</span><span> </span><span>"uvx"</span><span>,</span><span>     </span><br></span><span><span>    </span><span>"args"</span><span>:</span><span> </span><span>[</span><span>"mcp-server-time"</span><span>,</span><span> </span><span>"--local-timezone=America/New_York"</span><span>]</span><span>   </span><br></span><span><span>  </span><span>}</span><span> </span><br></span><span><span></span><span>}</span><br></span>
> ```
...
```
<span><span>uvx mcpo --port 8000 -- uvx mcp-server-time --local-timezone=America/New_York</span><br></span>
```
...
```
<span><span>FROM python:3.11-slim</span><br></span><span><span>WORKDIR /app</span><br></span><span><span>RUN pip install mcpo uv</span><br></span><span><span># Replace with your MCP server command; example: uvx mcp-server-time</span><br></span><span><span>CMD ["uvx", "mcpo", "--host", "0.0.0.0", "--port", "8000", "--", "uvx", "mcp-server-time", "--local-timezone=America/New_York"]</span><br></span>
```

2.  **Build & Run the Container Locally**

```
<span><span>docker build -t mcp-proxy-server .</span><br></span><span><span>docker run -d -p 8000:8000 mcp-proxy-server</span><br></span>
```
...
```
<span><span>docker tag mcp-proxy-server yourdockerusername/mcp-proxy-server:latest</span><br></span><span><span>docker push yourdockerusername/mcp-proxy-server:latest</span><br></span>
```
