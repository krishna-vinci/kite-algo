...
| Candle-close stages actually receive completed candles in production | Closed | `CandleAggregator._get_alert_tokens` — the aggregator additionally tracks the broker tokens of ACTIVE candle-close subscriptions | **Unit:** `tests/broker_api/test_candle_aggregator.py` (8 tests, incl. a positive control per lifecycle filter). **Live:** candles appeared 33s after redeploy; `consecutive_bars: 2` and a breadth crossing then fired |
