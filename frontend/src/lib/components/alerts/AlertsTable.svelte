<script lang="ts">
	...
	const sseUrl = API_BASE.replace(/\/$/, '') + '/api/alerts/events';
	...
	async function resolveSymbol(tok: number, tradingSymbol: string) {
		try {
			...
			try {
				const res = await apiFetch(`/api/instruments/by-token?token=${encodeURIComponent(tok)}`);
				if (res.ok) {
			...
			if (!sym) {
				const res = await apiFetch(
					`/api/instruments/fuzzy-search?query=${encodeURIComponent(tradingSymbol)}`
				);
				if (res.ok) {
					...
					const hit = Array.isArray(list)
						? (list.find((r) => Number(r.instrument_token) === tok) ?? list[0])
						: null;
					...
					if (evt?.type === 'alert.triggered' && evt?.id) {
						items = items.map((it) =>
							it.id === evt.id
								? {
										...it,
										status: evt.status || 'triggered',
										triggered_at: evt.triggered_at ?? it.triggered_at
									}
								: it
						);
		...
		<Table>
				...
				{:else}
					{#each items as a}
						<TableRow>
							<TableCell
								>{a.tradingsymbol ||
									(a.instrument_token ? symbolForToken(a.instrument_token) : '-')}</TableCell
							>
							<TableCell>{@html cmpSymbol(a.comparator)}</TableCell>
							<TableCell>{a.absolute_target ?? '-'}</TableCell>
							<TableCell
								>{$marketwatch.instruments[a.instrument_token]?.last_price ?? '-'}</TableCell
							>
							<TableCell>{a.baseline_price ?? '-'}</TableCell>
							<TableCell class="whitespace-nowrap"
								>{new Date(a.created_at).toLocaleString()}</TableCell
							>
							<TableCell>
								<span
									class={'inline-block rounded-full px-2 py-0.5 text-xs ' + statusClasses(a.status)}
									>{a.status}</span
								>
							</TableCell>
							<TableCell class="text-right">
								<div class="flex justify-end gap-2">
									...
									{/if}
									<button class="btn btn-xs" on:click={() => dispatch('recreate', { alert: a })}
										>Recreate</button
									>
			...
			<TableCaption class="text-left">
				<div class="flex items-center justify-between py-2">
					...
					<div class="flex items-center gap-2">
						<button class="btn btn-secondary btn-sm" on:click={prevPage} disabled={offset === 0}
							>Prev</button
						>
						<button
							class="btn btn-secondary btn-sm"
							on:click={nextPage}
							disabled={offset + limit >= total}>Next</button
						>
					</div>
				</div>
			</TableCaption>
