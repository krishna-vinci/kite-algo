  ...
  async function createAlert() {
    ...
    try {
      ...
      if (!validateRes.ok || v?.valid === false) {
        throw new Error(v?.reason ? `Validation failed: ${v.reason}` : `Validation failed (${validateRes.status})`);
      }
...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" on:click={() => dispatch('close')}>
  <div class="w-full max-w-3xl rounded-lg bg-white p-8 shadow-xl" on:click|stopPropagation>
    ...
    <div class="space-y-6">
      <input class="input w-full" bind:value={name} placeholder="Alert name, e.g., NIFTY crossing 25000" />
      ...
      <div class="flex items-end gap-4">
        ...
        <div class="field w-1/4">
          ...
          {:else}
            <div class="flex h-[42px] items-end gap-2">
              ...
              <button class="text-xs text-blue-600 hover:underline" on:click={() => (selectedInstrument = null)}>Change</button>
            </div>
          {/if}
        </div>
        ...
        <div class="field w-1/4">
          <input class="input" type="number" step="any" bind:value={rhs_constant} placeholder="e.g., 25000" />
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 pt-4">
        ...
        <label class="field">
          ...
          <input class="input" type="number" step="any" bind:value={percentOfLast} on:input={recomputeFromPercent} placeholder="e.g., 1.5" />
        </label>
      </div>
    </div>
