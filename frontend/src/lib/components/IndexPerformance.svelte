<script lang="ts">
			...
			if (response.ok) {
				searchResults = (await response.json()).filter(
					(instrument: Instrument) =>
						instrument.tradingsymbol && !selectedIndices.includes(instrument.tradingsymbol)
				);
			} else {
				searchResults = [];
			}
	...
	const getCellClass = (value: string) => {
		if (!value || value === 'N/A' || value === 'Data not available' || value === 'Error')
	...
	async function openHistory(index: string, period: string = '1Y') {
		...
		let url = `/api/candles/NSE:${encodeURIComponent(index)}?timeframe=day&ingest=false&passthrough=true`;
		if (fromDate) {
			url += `&from=${fromDate}`;
		}
		...
		try {
			...
			if (response.ok) {
				...
				const volumes = data.candles.map((c: any, i: number) => [
					i,
					c.volume,
					c.open > c.close ? 1 : -1
				]);
		...
		try {
			...
			const promises = compareSelection.map(async (idx, i) => {
				const response = await apiFetch(
					`/api/candles/NSE:${encodeURIComponent(idx)}?timeframe=day&ingest=false&passthrough=true`
				);
				if (response.ok) {
					...
					if (i === 0) {
						// Use dates from first series (assuming alignment)
	...
	<div class="flex justify-between items-center">
		<div class="flex items-center gap-2">
			...
			<Button
				variant="outline"
				size="icon"
				on:click={() => loadPerformanceData(true)}
				disabled={loading}
				title="Refresh Data"
			>
		...
		<div class="relative w-64">
			...
			{#if searchResults.length > 0}
				<ul
					class="absolute z-20 bg-popover text-popover-foreground border rounded-md mt-1 w-full shadow-md max-h-60 overflow-auto"
				>
					{#each searchResults as instrument (instrument.instrument_token)}
						<li
							on:click={() => addIndex(instrument)}
						...
						>
							...
							{#if performanceData[index]}
								{#each ['1D', '1W', '1M', '3M', '6M', '1Y'] as period}
									...
									<td
										class="p-4 align-middle text-center cursor-pointer hover:opacity-80 transition-opacity {getCellClass(
											performanceData[index][period]
										)}"
							...
							<td class="p-4 align-middle text-center">
								<button
									...
									class="text-muted-foreground hover:text-destructive invisible group-hover:visible transition-colors"
								>
									{@html CloseIcon}
								</button>
							</td>
