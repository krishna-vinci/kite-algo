...
export default function JournalAnalyticsCostsPage() {
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
                  ...
                  <th className="pb-2 text-right font-medium">Cost / Ep</th>
                  <th className="pb-2 text-right font-medium">Ratio</th>
                </tr>
              </thead>
              <tbody>
                {data.strategies.map((s) => {
                  const totalCharges = Number(toNum(s.total_charges) ?? 0);
                  const costPerEpisode = s.closed_episode_count > 0 ? totalCharges / s.closed_episode_count : null;
                  ...
                  <tr key={s.strategy.template_id} className="border-t border-border/40">
                    ...
                    <td className="py-1.5 text-right tabular-nums text-muted-foreground">
                      {costPerEpisode === null ? "—" : `₹${costPerEpisode.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                    </td>
                    <td className="py-1.5 text-right tabular-nums text-muted-foreground">
