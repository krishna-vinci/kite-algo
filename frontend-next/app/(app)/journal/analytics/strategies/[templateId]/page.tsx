...
function WinLossDistribution({ winCount, lossCount }: { winCount: number; lossCount: number }) {
  const total = winCount + lossCount;
  const winWidth = total > 0 ? (winCount / total) * 100 : 0;
  const lossWidth = total > 0 ? (lossCount / total) * 100 : 0;

  return (
    <div className="rounded-xl border border-border/60 bg-background/40 p-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-[10px] uppercase tracking-[0.24em] text-muted-foreground">Win / Loss Distribution</p>
      ...
      <div className="mt-3 h-3 overflow-hidden rounded-full bg-muted/40">
        <div className="flex h-full w-full">
          <div className="bg-[var(--green)]/80" style={{ width: `${winWidth}%` }} />
          <div className="bg-[var(--red)]/80" style={{ width: `${lossWidth}%` }} />
        </div>
      </div>
      <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
        <span>{total > 0 ? `${winWidth.toFixed(1)}% win` : "—"}</span>
        <span>{total > 0 ? `${lossWidth.toFixed(1)}% loss` : "—"}</span>
      </div>
    </div>
  );
}

export default function JournalStrategyDeepDivePage() {
  ...
  const backHref = `/journal/analytics${backSp.toString() ? `?${backSp.toString()}` : ""}`;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <WinLossDistribution winCount={data.metrics.win_count} lossCount={data.metrics.loss_count} />
