...
#                asyncio.run_coroutine_threadsafe(self._process_ticks_async(ticks), self.loop)
#        except Exception as e:
...
#        try:
#            timeframe = INTERVAL_SECONDS.get(interval, 60)
#            if timeframe == 'day':
#                ttl_seconds = 86400 * 2  # 2 days
#            else:
#                ttl_seconds = int(timeframe) * 2
#            await self.redis.set(key, json.dumps(candle_list), ex=ttl_seconds)
#        except Exception as e:
...
#     to ensure the latest candles are retrieved even on large lookbacks.
#     """
#     all_records = []
...
#     Handles both timezone-aware and naive datetime objects from the API.
#     """
#     ts = record['date']
#     ist = pytz.timezone('Asia/Kolkata')
    
#     # Kiteconnect can return naive datetimes which are in IST.
#     # If naive, we must first localize to IST.
...
# @router.delete("/historical/cache/{identifier}", summary="Clear all cached candle data for an instrument")
# async def clear_instrument_cache(
#     identifier: str,
#     db: Session = Depends(get_db)
# ):
#     """
#     Deletes all rows from the historical_candles table for a specific instrument,
#     effectively clearing its cache across all timeframes.
#     """
...
#         conn = get_db_connection()
#         with conn.cursor() as cur:
#             query = "DELETE FROM public.historical_candles WHERE instrument_token = %s;"
#             cur.execute(query, (instrument_token,))
#             deleted_count = cur.rowcount
#             conn.commit()
            
#         logger.info(f"Cleared cache for instrument_token {instrument_token} ('{identifier}'). Deleted {deleted_count} rows.")
#         return {
#             "status": "success",
...
#         if conn:
#             conn.rollback()
#         logger.error(f"DB error clearing cache for {identifier}: {e}", exc_info=True)
#         raise HTTPException(status_code=500, detail=f"Database error while clearing cache: {e}")
#     finally:
#         if conn:
#             conn.close()
...
#                 # If the datetime object is naive, assume it's in IST and make it aware.
#                 if timestamp.tzinfo is None:
#                     timestamp = ist_tz.localize(timestamp)
#                 # If it's already aware, ensure it's converted to IST.
#                 else:
...
# TIMEFRAME_ALIASES = {
#     "1m": "minute", "min": "minute", "minute": "minute", "1minute": "minute",
...
#     from_ist = from_date.astimezone(IST) if from_date.tzinfo else IST.localize(from_date)
#     to_ist = to_date.astimezone(IST) if to_date.tzinfo else IST.localize(to_date)
    
#     logger.info(f"BG Ingestion: Starting for {instrument_token}|{interval} from {from_ist} to {to_ist}")
#     try:
#         records = await fetch_historical_data_chunked(
#             kite, instrument_token, from_ist, to_ist, interval, False, False
#         )
#         if records:
#             db_rows = [convert_kite_record_to_ist_tuple(r) for r in records]
...
#         to_utc = to_date.astimezone(timezone.utc) if to_date.tzinfo else to_date.replace(tzinfo=timezone.utc)
#     else:
#         # Use current time in IST, convert to UTC for queries
#         now_ist = datetime.now(IST)
#         to_utc = now_ist.astimezone(timezone.utc)
#         logger.info(f"No to_date provided, using current time: {now_ist} IST = {to_utc} UTC")
...
#         missing_from_utc = from_utc
#         if last_ts:
#             # DB returns IST timestamp, convert to UTC for comparison
#             last_ts_utc = last_ts.astimezone(timezone.utc) if last_ts.tzinfo else IST.localize(last_ts).astimezone(timezone.utc)
#             # Start check from the candle *after* the last stored one
#             missing_from_utc = max(from_utc, last_ts_utc + TIMEFRAME_TIMEDELTA[interval])
#             logger.info(f"Last timestamp in DB for {instrument_token}|{interval}: {last_ts} (IST), converted to UTC: {last_ts_utc}, missing_from: {missing_from_utc}")
#         else:
#             logger.info(f"No existing data for {instrument_token}|{interval}, fetching from {from_utc} to {to_utc}")

#         if missing_from_utc < to_utc:
#             logger.info(f"Triggering ingestion for {instrument_token}|{interval} from {missing_from_utc} to {to_utc}")
#             background_tasks.add_task(
#                 trigger_historical_ingestion, kite, instrument_token, interval, missing_from_utc, to_utc
#             )
#             ingestion_status = "triggered"
#         else:
#             logger.info(f"Data up-to-date for {instrument_token}|{interval}. Last: {last_ts_utc}, requested to: {to_utc}")
#             ingestion_status = "up_to_date"

#     # 4. Query DB for the requested range and return
#     # Convert UTC query params to IST since DB stores in IST
...
#             rows = cur.fetchall()
#             logger.info(f"Query returned {len(rows)} rows for {instrument_token}|{interval} between {from_ist} and {to_ist}")
...
#         # Use already converted IST times for metadata
