...
"""Preview grouped live exit without placing broker orders."""
...
def main() -> None:
    run_id = os.environ["KITE_ALGO_RUN_ID"]
    ...
    preview = client.exit_run(
        ...
        idempotency_key=f"{run_id}:exit-preview:{os.getenv('KITE_ALGO_EXIT_PREVIEW_ID', '001')}",
        dry_run=True,
    )
    print(preview)
